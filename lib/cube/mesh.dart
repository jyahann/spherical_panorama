...
class Mesh {
  Mesh({List<Vector3>? vertices, List<Offset>? texcoords, List<Polygon>? indices, List<Color>? colors, this.texture, Rect? textureRect, this.texturePath, CubeMaterial? material, this.name}) {
    this.vertices = vertices ?? <Vector3>[];
    this.texcoords = texcoords ?? <Offset>[];
    this.colors = colors ?? <Color>[];
    this.indices = indices ?? <Polygon>[];
    this.material = material ?? CubeMaterial();
...
Future<List<Mesh>> loadObj(String fileName, bool normalized, {bool isAsset = true}) async {
  Map<String, CubeMaterial>? materials;
  ...
  List<String> elementCubeMaterials = <String>[];
  ...
  for (var line in lines) {
    ...
    switch (parts[0]) {
      ...
      case 'usemtl':
        ...
        elementCubeMaterials.add(materialName ?? '');
...
Future<List<Mesh>> _buildMesh(
  ...
  List<String> elementCubeMaterials,
  ...
  for (int index = 0; index < elementOffsets.length; index++) {
    ...
    final CubeMaterial? material = (materials != null) ? materials[elementCubeMaterials[index]] : null;
